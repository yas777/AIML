...
def generate2optNeighbours(tour):
    ...
    for i in range(1, cities + 1):
        for j in range(i+2, cities + 1):
            ...
            temp2.reverse()
            all_possible_neighbours.append(temp1 + temp2 + temp3)
    ...
    for i in range(1, cities + 1):
        for j in range(i+2, cities + 1):
            for k in range(j+2, cities + 1):
                ...
                temp2.reverse()
                temp3.reverse()
                all_possible_neighbours.append(temp1 + temp2 + temp3 + temp4)
                temp2.reverse()
                temp3.reverse()
                all_possible_neighbours.append(temp1 + temp3 + temp2 + temp4)
                temp2.reverse()
                all_possible_neighbours.append(temp1 + temp3 + temp2 + temp4)
                temp3.reverse()
                temp2.reverse()
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    def hillclimbUtil(tour):
        min_len = float("inf")
        for i in getNeighbours(tour):
            tour_len = getTourLength(i)
            if (min_len > tour_len):
                min_len = tour_len
                final_tour = i
        return min_len, final_tour

    tl, tr = hillclimbUtil(initial_tour)
    while(tl <= min_tl):
        tourLengthList.append(tl)
        min_tl = tl
        minTour = tr
        # print(min_tl)
        tl, tr = hillclimbUtil(tr)
    
    
    # tourLengthList.append(min_tl)
    return tourLengthList, minTour
...
def nearestNeighbourTour(initial_city):
    ...
    while(c != cities):
        ...
        for j in range(1, cities + 1):
            if(visited[j] == 0 and j != city):
                len = getDistance(nodeDict[city], nodeDict[j])
                if (len < min_len):
                    min_len = len
                    temp = j
        c = c + 1
