...
class SudokuSearchProblem:
    ...
    def isGoalState(self, state):
        ...
        """
        return (all(len(d) == 1 for s,d in state.items()) and solved(state))
    ...
    def getSuccessors(self, state):
        ...
        min_val = min(len(d) for s,d in state.items() if(len(d) > 1))
        for s,d in state.items():
            if (len(d) == min_val):
                for c in d:
                    if(assign(state.copy(), s, c)):
                        successors.append((assign(state.copy(), s, c), (s, c), 1))
                break
        ## YOUR CODE HERE
