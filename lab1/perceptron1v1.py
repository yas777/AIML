	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for label1 in range(len(self.legalLabels)):
					lab1 = self.legalLabels[label1]
					if (lab1 != act):					
						if (lab1 < act):	
							if ((self.weights[frozenset([act,lab1])] * trainingData[i]) <= 0):
								self.weights[frozenset([act,lab1])] = self.weights[frozenset([act,lab1])] + trainingData[i];
						else:
							if ((self.weights[frozenset([lab1,act])] * trainingData[i]) > 0):
								self.weights[frozenset([lab1,act])] = self.weights[frozenset([lab1, act])] - trainingData[i]
