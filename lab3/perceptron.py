...
class PerceptronClassifier:
    ...
    def train( self, trainingData, trainingLabels, sample_weights=None):
        ...
        for iteration in range(self.max_iterations):
            for i in range(len(trainingData)): 
                act_lab = trainingLabels[i]
                # exp_lab = self.weights*trainingData[i]
                exp_lab = self.classify([trainingData[i]])[0]
                if (exp_lab != act_lab):
                    if (act_lab > 0):
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        td = np.asarray(trainingData)
        tl = np.asarray(trainingLabels)
        n = td.shape[0]
        ind = np.random.choice(n,int(0.5*n),p = sample_weights)
        out_data = np.take(td,ind)
        out_label = np.take(tl,ind)
