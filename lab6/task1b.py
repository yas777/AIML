...
trs = {i:{j:[] for j in range(na)} for i in range(ns)}
stact = {i:[] for i in range(ns)}
while(1):
    temp = f.readline()
    arr = temp.split()
    if(arr[0] != "discount"):
        ...
        ts[st0][act][st1] = t
        rw[st0][act][st1] = r
        stact[st0].append(act)
        trs[st0][act].append([st1, r, t])
    else:
        disc = float(arr[1])
        break
    ...
    for i in range(ns):
        ...
        if(eds is None or eds[i] == False):
            for j in stact[i]:
                sig = 0.0
                flag = 0
                for st1, rw1, t1 in trs[i][j]:
                    flag = 1
                    sig+=((t1)*(rw1 + disc*(prevl[st1])))
                # for k in range(ns):
                #     if(ts[i][j][k] != 0):
                #         flag = 1 
